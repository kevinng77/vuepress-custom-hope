...
def get_articles(source_path, output_path):
    files = []
    for file in os.listdir(source_path):  # no math
        if file.endswith(".md"):
            source_file = source_path + file
            output_file = output_path + file
            files.append((source_file, output_file))
        elif os.path.isdir(source_path + file):
            for inner_files in os.listdir(source_path + file):
                if inner_files.endswith(".md"):
                    source_file = source_path + file + inner_files
                    output_file = output_path + file + inner_files
                    files.append((source_file, output_file))
        
    return files

def main(process_pipelines=[]):
    ...
    """
    args = get_args()
    source_path, output_path = args.source_path, args.output_path
    ...
    files = get_articles(source_path, output_path)
    for source_file, output_file in files:
        print(source_file)
    for file in os.listdir(source_path): 
        if file.endswith(".md"):
            source_file = source_path + file
            ...
            if (cur_time - file_time).days < args.days:
                print(f"处理文章： {file}, \t修改时间: {file_time}")
...
def get_args():
    ...
    parser.add_argument("--source_path",default="/mnt/together/nut/_posts/",type=str)
    parser.add_argument("--output_path",default="../cleaned_posts/")
    parser.add_argument("--name",default="",type=str)
    return parser.parse_args()


if __name__ == '__main__':
    process_pipelines = [img_path_edit,
                         bolder_edit,
                         sep_ch_and_en, 
                         code_block_edit,
                         centerIMG]
    main(process_pipelines=process_pipelines)