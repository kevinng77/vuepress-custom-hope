...
def edit_article(source_file, output_file, process_pipelines):
    text = []
...
def main(source_path, output_path=None, process_pipelines=[]):
    ...
    for i in range(len(source_path)):
        for file in os.listdir(source_path[i]):  # no math
            file_time = get_FileModifyTime(file)
            if file.endswith(".md"):
                print(f"processing {file}, create time: {file_time}")
...
if __name__ == '__main__':
    source_path = "/mnt/together/nut/_post/"
    ...
    main(source_path = output_path, process_pipelines=process_pipelines)
